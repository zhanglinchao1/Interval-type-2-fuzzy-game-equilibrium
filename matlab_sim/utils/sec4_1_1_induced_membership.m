...
%
%   可选压力测试扩展:
%       params.fou_strategy_scale 为 1×4 或 4×1 向量时, bell-shaped FOU 半宽
%       额外乘以 pi_profile * fou_strategy_scale, 用于建模不同策略暴露程度。
%       默认不启用, 不改变原实验。
%
%   输入:
%       pi_profile - N×4 矩阵
...
%       mu_upper_pi - N×3 矩阵, 上界
    ...
    if use_bell_fou
        ...
        if isfield(params, 'fou_strategy_scale') && ...
            ...
            exposure = pi_profile * strategy_scale;
            delta_eff = delta_eff .* exposure;
        end
